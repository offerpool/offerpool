import { addOfferEntryToDB } from "./add-offer-to-db.js";
...
export const updateLocalDB = async (db: any, starting: any) => {
  if (updateInProgress) {
  ...
  while (
    ...
    for (let i = 0; i < offerArray.length; i++) {
      if (!offersThatAlreadyExist[i]) {
        await addOfferEntryToDB(offerArray[i]);
        offersAdded++;
      }
    }
