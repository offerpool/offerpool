...
export const TakeOfferInGoby = ({ offer, account }) => {
  ...
  async function takeOffer() {
    try {
      await window.chia.request({ method: "takeOffer", params: { offer: offer.offer, fee: 0 } });
    } catch (err) {
  ...
  return (
    ...
    <ThemeContext.Consumer>
      {({ theme }) => (
        <button onClick={takeOffer} className="btn-link-secondary goby-button" title={t`take offer in goby`}>
          <img
            src={
